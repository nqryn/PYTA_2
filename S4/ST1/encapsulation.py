"""
Encapsulation = notiunea de a tine toate atributele si metodele relevante pentru un obiect
in clasa acestuia, si de a le afisa/ascunde in functie de nevoi.
...
Avem 3 tipuri de modificatori care ne dau vizibilitatea atributelor si metodelor:
 - privat : atribute si metode care NU sunt deloc vizibile in afara clasei in care sunt definite
    acestea incep cu dublu underscore
...
class Person:
    ...
    # acesta se numeste getter, deoarece expune valoarea privata in afara clasei
    @property   # decoratorul property imi transforma o metoda intr-un atribut
    def cnp(self):
...
ion.describe()
# print(ion.__cnp)      # va da AttributeError: 'Person' object has no attribute '__cnp'
ion.__cnp = 'cnp nou'   # nu va modifica __cnp, deoarece nu il poate "vedea"
ion.describe()

print(ion.cnp)
ion.cnp = 'altceva nou' # va da eroare daca nu avem setter AttributeError: can't set attribute 'cnp'
ion.describe()

# va da eroare AttributeError: 'Person' object has no attribute '__private_method'. Did you mean: '_Person__private_method'?
# ion.__private_method()

print('_' * 80)
print(ion.name, ion._name)                # va afisa numele, dar nu ar trebui sa accesam in felul acesta
ion.name = 'Gheorghe Popescu'      # va merge, dar nu ar trebui sa facem asa.
ion.describe()
