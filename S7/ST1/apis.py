...
Este un standard care ne permite sa realizam comunicarea intre anumite aplicatii.
In mod usual, noi ne vom referi la o arhitectura de tipul client-server.

Client = aplicatia care "consuma" API-ul, adica cea care cere informatii prin API
de la un server.
=> cel mai des clientul nostru va fi un browser (Chrome, Firefox) sau o app mobila

Server = aplicatie care "produce" un API, adica cea care ofera date pentru un client.


REST = REpresentational Stateless Transfer
Este un set de constrangeri arhitecturale care permite ca aceasta comunicare
client-server sa se realizeze cat mai usor cu putinta.
